export const STAT_CATEGORIES = {
  offense: {
    ...
    color: '#f5a623',
    stats: [
      { key: 'td_passing',     label: 'Passing TD',         icon: 'td_passing',     unit: null,   description: 'Touchdown pass (QB)',                    flag: true,  contact: true },
      ...
      { key: 'two_pt_carry',   label: '2PT Conv Carry',     icon: 'two_pt_carry',   unit: null,   description: '2-pt conversion run',                   flag: true,  contact: true },
      { key: 'one_pt_pass',    label: '1PT Conv Pass',      icon: 'one_pt_pass',    unit: null,   description: '1-pt conversion pass (QB)',              flag: true,  contact: true },
      { key: 'one_pt_rec',     label: '1PT Conv Reception', icon: 'one_pt_rec',     unit: null,   description: '1-pt conversion catch',                 flag: true,  contact: true, excludeFromStatFirst: true },
      { key: 'one_pt_carry',   label: '1PT Conv Carry',     icon: 'one_pt_carry',   unit: null,   description: '1-pt conversion run',                   flag: true,  contact: true },
    ]
  },
  defense: {
    label: 'Defense',
    color: '#4a90d9',
    stats: [
      { key: 'tackle',         label: 'Tackle',             icon: 'tackle',         unit: null,   description: 'Solo tackle / flag pull',               flag: true,  contact: true },
      { key: 'tackle_assist',  label: 'Tackle Assist',      icon: 'tackle_assist',  unit: null,   description: 'Assisted tackle',                       flag: false, contact: true, show_more: true },
      { key: 'tfl',            label: 'TFL',                icon: 'tfl',            unit: null,   description: 'Tackle for loss',                       flag: true,  contact: true },
      { key: 'sack',           label: 'Sack',               icon: 'sack',           unit: null,   description: 'QB sack',                               flag: true,  contact: true },
  ...
  special: {
    ...
    color: '#6edba8',
    stats: [
      { key: 'kick_return_yds', label: 'Kick Return Yds',  icon: 'kick_return_yds', unit: 'yds',  description: 'Kick return yards',                     flag: false, contact: true, show_more: true },
      { key: 'punt_return_yds', label: 'Punt Return Yds',  icon: 'punt_return_yds', unit: 'yds',  description: 'Punt return yards',                     flag: false, contact: true, show_more: true },
      { key: 'pat_kick',        label: 'PAT Kick',         icon: 'pat_kick',         unit: null,   description: 'Extra point kick',                      flag: false, contact: true, show_more: true },
      { key: 'field_goal',      label: 'Field Goal',       icon: 'field_goal',       unit: null,   description: 'Field goal made',                       flag: false, contact: true, show_more: true },
    ]
  }
};
...
export function getStatInfo(key) {
  return ALL_STATS.find(s => s.key === key) || { key, label: key, icon: key, unit: null };
}
...
export function getStatNarrative(stat, allStats) {
  ...
  switch (stat.stat_type) {
    ...
    case 'return_2pt':
      return `${name} returned for a 2PT conversion`;
    case 'return_1pt':
      return `${name} returned for a 1PT conversion`;
    case 'two_pt_pass': {
      const rec = findPaired('two_pt_rec');
      return rec
        ? `${name} converted the 2PT pass to #${rec.player_number} ${rec.player_name}`
        : `${name} threw a 2PT conversion pass`;
    }
    case 'two_pt_rec': {
      const pass = findPaired('two_pt_pass');
      return pass ? null : `${name} caught a 2PT conversion`;
    }
    case 'two_pt_carry':
      return `${name} ran in a 2PT conversion`;
    case 'one_pt_pass': {
      const rec = findPaired('one_pt_rec');
      return rec
        ? `${name} converted the PAT pass to #${rec.player_number} ${rec.player_name}`
        : `${name} threw a PAT pass`;
    }
    case 'one_pt_rec': {
      const pass = findPaired('one_pt_pass');
      return pass ? null : `${name} caught a PAT conversion`;
    }
    case 'one_pt_carry':
    ...
    case 'punt_return_yds':
      return `${name} returned the punt for ${val} yards`;
    case 'pat_kick':
